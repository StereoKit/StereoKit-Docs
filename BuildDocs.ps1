...
dotnet build
dotnet run -- --xml "$SKFolder/bin/netstandard2.0/StereoKit.xml" --library "$SKFolder/bin/netstandard2.0/StereoKit.dll" --samples "$SKFolder/Examples/StereoKitTest/" --out "$PSScriptRoot/docs/Pages/" --ai-out "$PSScriptRoot/docs/"
Pop-Location