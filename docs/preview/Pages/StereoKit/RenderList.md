...
|  |  |
|--|--|
|int [Count]({{site.url}}/preview/Pages/StereoKit/RenderList/Count.html)|The number of Mesh/Material pairs that have been submitted to the render list so far this frame.|
|string [Id]({{site.url}}/preview/Pages/StereoKit/RenderList/Id.html)|Gets or sets the unique identifier of this asset resource! This can be helpful for debugging, managing your assets, or finding them later on!|
|int [PrevCount]({{site.url}}/preview/Pages/StereoKit/RenderList/PrevCount.html)|This is the number of items in the RenderList before it was most recently cleared. If this is a list that is drawn and cleared each frame, you can think of this as "last frame's count".|

## Instance Methods

|  |  |
|--|--|
|[RenderList]({{site.url}}/preview/Pages/StereoKit/RenderList/RenderList.html)|Creates a new empty RenderList.|
|[Add]({{site.url}}/preview/Pages/StereoKit/RenderList/Add.html)|Add a Mesh/Material to the RenderList. The RenderList will hold a reference to these Assets until the list is cleared.|
|[Clear]({{site.url}}/preview/Pages/StereoKit/RenderList/Clear.html)|Clears out and de-references all Draw items currently in the RenderList.|
|[DrawNow]({{site.url}}/preview/Pages/StereoKit/RenderList/DrawNow.html)|Draws the RenderList to a rendertarget texture immediately. It does _not_ clear the list.|

## Static Fields and Properties

|  |  |
|--|--|
|[RenderList]({{site.url}}/preview/Pages/StereoKit/RenderList.html) [Primary]({{site.url}}/preview/Pages/StereoKit/RenderList/Primary.html)|The default RenderList used by the Renderer for the primary display surface.|

## Static Methods

|  |  |
|--|--|
|[Find]({{site.url}}/preview/Pages/StereoKit/RenderList/Find.html)|Finds the RenderList with the matching id, and returns a reference to it. If no RenderList is found, it returns null.|
|[Pop]({{site.url}}/preview/Pages/StereoKit/RenderList/Pop.html)|This removes the current top of the RenderList stack, making the next list as active|
|[Push]({{site.url}}/preview/Pages/StereoKit/RenderList/Push.html)|All draw calls that don't specify a render list will get submitted to the active RenderList at the top of the stack. By default, that's RenderList.Primary, but you can push your own list onto the stack here to capture draw calls, like those done in the UI.|
