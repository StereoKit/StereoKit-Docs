...
|  |  |
|--|--|
|bool [Enabled]({{site.url}}/preview/Pages/StereoKit/Hierarchy/Enabled.html)|This is enabled by default. Disabling this will cause any draw call to ignore any Matrices that are on the Hierarchy stack.|

## Static Methods

|  |  |
|--|--|
|[Pop]({{site.url}}/preview/Pages/StereoKit/Hierarchy/Pop.html)|Removes the top Matrix from the stack!|
|[Push]({{site.url}}/preview/Pages/StereoKit/Hierarchy/Push.html)|Pushes a transform Matrix onto the stack, and combines it with the Matrix below it. Any draw operation's Matrix will now be combined with this Matrix to make it relative to the current hierarchy. Use Hierarchy.Pop to remove it from the Hierarchy stack! All Push calls must have an accompanying Pop call.|
|[ToLocal]({{site.url}}/preview/Pages/StereoKit/Hierarchy/ToLocal.html)|Converts a world space point into the local space of the current Hierarchy stack!|
|[ToLocalDirection]({{site.url}}/preview/Pages/StereoKit/Hierarchy/ToLocalDirection.html)|Converts a world space direction into the local space of the current Hierarchy stack! This excludes the translation component normally applied to vectors, so it's still a valid direction.|
|[ToWorld]({{site.url}}/preview/Pages/StereoKit/Hierarchy/ToWorld.html)|Converts a local point relative to the current hierarchy stack into world space!|
|[ToWorldDirection]({{site.url}}/preview/Pages/StereoKit/Hierarchy/ToWorldDirection.html)|Converts a local direction relative to the current hierarchy stack into world space! This excludes the translation component normally applied to vectors, so it's still a valid direction.|
...
![Two spheres, one red and one blue, both at different locations]({{site.url}}/preview/img/screenshots/Docs/Hierarchy_Transform.jpg)
...
![An intersecting Ray in a complicated hierarchy]({{site.url}}/preview/img/screenshots/Docs/Hierarchy_Spaces.jpg)
