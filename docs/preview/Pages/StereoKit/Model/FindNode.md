...
description: Searches the entire list of Nodes, and will return the first on that matches this name exactly. If no ModelNode is found, then this will return null. Node Names are not guaranteed to be unique. This will block until the Model's metadata is loaded.
---
# [Model]({{site.url}}/preview/Pages/StereoKit/Model.html).FindNode
...
|RETURNS: [ModelNode]({{site.url}}/preview/Pages/StereoKit/ModelNode.html)|The first matching ModelNode, or null if none are found.|
