---
layout: default
title: Input.KeyboardEventAt
description: Reads a keyboard event by index, without consuming anything. This suits code that wants to observe the frame's input even after something else, like a focused UI.Input, has consumed the queue.
---
# [Input]({{site.url}}/preview/Pages/StereoKit/Input.html).KeyboardEventAt

<div class='signature' markdown='1'>
```csharp
static KeyboardEvent KeyboardEventAt(int index)
```
Reads a keyboard event by index, without consuming
...
|  |  |
|--|--|
|int index|Index of the event, from 0 up to `Input.KeyboardEventCount`.|
|RETURNS: [KeyboardEvent]({{site.url}}/preview/Pages/StereoKit/KeyboardEvent.html)|The event at that index, or an event of type `KeyboardEventType.None` if the index is out of range.|
