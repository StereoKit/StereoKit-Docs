---
layout: default
title: Ray.Closest
description: Calculates the point on the Ray that's closest to the given point! This will be clamped if the point is behind the ray's origin.
---
# [Ray]({{site.url}}/preview/Pages/StereoKit/Ray.html).Closest

<div class='signature' markdown='1'>
```csharp
Vec3 Closest(Vec3 to)
```
Calculates the point on the Ray that's closest to the
given point! This will be clamped if the point is behind the ray's
origin.
</div>

|  |  |
|--|--|
|[Vec3]({{site.url}}/preview/Pages/StereoKit/Vec3.html) to|Any point in the same coordinate space as the             Ray.|
|RETURNS: [Vec3]({{site.url}}/preview/Pages/StereoKit/Vec3.html)|The point on the ray that's closest to the given point.|




