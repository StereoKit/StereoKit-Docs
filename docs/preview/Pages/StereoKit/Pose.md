...
|  |  |
|--|--|
|[Vec3]({{site.url}}/preview/Pages/StereoKit/Vec3.html) [Forward]({{site.url}}/preview/Pages/StereoKit/Pose/Forward.html)|Calculates the forward direction from this pose. This is done by multiplying the orientation with Vec3.Forward. Remember that Forward points down the -Z axis!|
|[Quat]({{site.url}}/preview/Pages/StereoKit/Quat.html) [orientation]({{site.url}}/preview/Pages/StereoKit/Pose/orientation.html)|Orientation of the pose, stored as a rotation from Vec3.Forward.|
|[Vec3]({{site.url}}/preview/Pages/StereoKit/Vec3.html) [position]({{site.url}}/preview/Pages/StereoKit/Pose/position.html)|Location of the pose.|
|[Ray]({{site.url}}/preview/Pages/StereoKit/Ray.html) [Ray]({{site.url}}/preview/Pages/StereoKit/Pose/Ray.html)|This creates a ray starting at the Pose's position, and pointing in the 'Forward' direction. The Ray direction is a unit vector/normalized.|
|[Vec3]({{site.url}}/preview/Pages/StereoKit/Vec3.html) [Right]({{site.url}}/preview/Pages/StereoKit/Pose/Right.html)|Calculates the right (+X) direction from this pose. This is done by multiplying the orientation with Vec3.Right.|
|[Vec3]({{site.url}}/preview/Pages/StereoKit/Vec3.html) [Up]({{site.url}}/preview/Pages/StereoKit/Pose/Up.html)|Calculates the up (+Y) direction from this pose. This is done by multiplying the orientation with Vec3.Up.|

## Instance Methods

|  |  |
|--|--|
|[Pose]({{site.url}}/preview/Pages/StereoKit/Pose/Pose.html)|Basic initialization constructor! Just copies in the provided values directly.|
|[ToMatrix]({{site.url}}/preview/Pages/StereoKit/Pose/ToMatrix.html)|Converts this pose into a transform matrix, incorporating a provided scale value.|
|[ToMatrixInv]({{site.url}}/preview/Pages/StereoKit/Pose/ToMatrixInv.html)|Converts this pose into the inverse of the Pose's transform matrix. This can be used to transform points from the space represented by the Pose into world space.|
|[ToString]({{site.url}}/preview/Pages/StereoKit/Pose/ToString.html)|A string representation of the Pose, in the format of "position, Forward". Mostly for debug visualization.|

## Static Fields and Properties

|  |  |
|--|--|
|[Pose]({{site.url}}/preview/Pages/StereoKit/Pose.html) [Identity]({{site.url}}/preview/Pages/StereoKit/Pose/Identity.html)|A default, empty pose. Positioned at zero, and using Quat.Identity for orientation.|

## Static Methods

|  |  |
|--|--|
|[Lerp]({{site.url}}/preview/Pages/StereoKit/Pose/Lerp.html)|Interpolates between two poses! It is unclamped, so values outside of (0,1) will extrapolate their position.|
|[LookAt]({{site.url}}/preview/Pages/StereoKit/Pose/LookAt.html)|Creates a Pose that looks from one location in the direction of another location. This leaves "Up" as the +Y axis.|
...
![Identity pose at the origin]({{site.url}}/preview/img/screenshots/Docs/PoseIdentity.jpg)
...
![Lerping between two poses]({{site.url}}/preview/img/screenshots/Docs/PoseLerp.jpg)

Here we construct two `Pose`s, one using a position + direction
constructor, and one using a from -> to LookAt function. Both are
valid ways of constructing a `Pose`, check out the [`Quat`]({{site.url}}/preview/Pages/StereoKit/Quat.html)
...
![Drawing items at a Pose]({{site.url}}/preview/img/screenshots/Docs/PoseDraw.jpg)
