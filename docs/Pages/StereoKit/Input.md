...
|[ControllerModelGet]({{site.url}}/Pages/StereoKit/Input/ControllerModelGet.html)|This retreives the Model currently in use by StereoKit to represent the controller input source. By default, this will be a Model provided by OpenXR, or SK's fallback Model. This will never be null while SK is initialized.|
|[ControllerModelSet]({{site.url}}/Pages/StereoKit/Input/ControllerModelSet.html)|When StereoKit is rendering the input source, this allows you to override the controller Model SK uses. The Model SK uses by default may be provided from the OpenXR runtime depending on extension support, but if not, SK does have a default Model. Setting this to null will restore SK's default.|
